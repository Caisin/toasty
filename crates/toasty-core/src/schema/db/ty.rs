    ...
    pub fn from_app(
        ...
        match hint {
            ...
            None => match ty {
                ...
                // An embedded model column or dynamic JSON value is stored as
                // a single document (`jsonb` on PG, `JSON` elsewhere) — never
                // a native array. `Type::list` collapses a list-of-documents
                // back to one document, so a `#[document]` collection
                // (`List(Model)`) lands here as a plain `Document`.
